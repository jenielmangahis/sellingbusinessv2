...
<div class="listing-item" 
	...
	<a href="<?php the_permalink(); ?>" class="listing-img-container">
		...
		<div class="listing-img-content">
			...
			<?php 
			$bmarks=realteo_get_option( 'bookmarks_page' );
			if(!empty($bmarks)) {
			...
			$cpage=realteo_get_option( 'compare_page' );
				if(!empty($cpage)) {
	...
	<div class="listing-content">
		...
		<div class="listing-title">
			...
			<a href="<?php the_permalink(); ?>" class="details button border"><?php esc_html_e('Enquire Now','realteo'); ?></a>
		</div>
		<?php 
		...
		if(!is_front_page()) {
		?>
		<div class="property-main-features" style="padding:15px 20px;columns:inherit;display:block;overflow:hidden"><?php the_excerpt(); ?>
		<?php
		}
		if(!is_front_page()) {
			$agency_id = getAgencyIdByAuthor($post->post_author);
			$img_id = get_post_meta( $agency_id, '_thumbnail_id', true );
			if($img_id){
			 	echo '<a href="' . get_permalink($agency_id) . '">'.wp_get_attachment_image($img_id, 'full', false, array('style'=>'height:70px;width:auto;float:right;display:block')). '</a>';
			}			
        ...
        <?php
			...
			if(!is_front_page() && realteo_get_option( 'realteo_hide_listing_footer', false ) == false){ ?>
