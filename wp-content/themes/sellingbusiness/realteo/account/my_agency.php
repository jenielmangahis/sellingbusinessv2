...
if($is_temp_agent) : 
	...
	<div class="stick-to-table">
		<h4><?php esc_html_e( "You've been invited to the following agencies", 'realteo' );	 ?></h4>
	</div>
...
if($is_agent) : 
	...
	if($has_agencies) : ?>
<div class="stick-to-table">
		<h4><?php esc_html_e( "You're assigned as agent to the following agencies", 'realteo' );	 ?></h4>
	</div>
...
if($agencies) : ?>
	<div class="stick-to-table">
		<h4><?php esc_html_e( 'Agencies that you can manage', 'realteo' );	 ?></h4>
	</div>
				...
				<td>
					...
					<?php if ( get_post_status ( $agency_id ) == 'publish' ) { ?>
						<div class="add-agent-2-agency-form">
							<span class="add-new-agent-title">Add New Agent/Broker</span>
							<form action="" id="search_agent_<?php echo $agency->ID ?>"  class="search_agent">
								<input  placeholder="Type broker's email or login" type="text" data-agency="<?php echo $agency->ID ?>" name="find_agents" id="add_agent"> <button class="button"><i class="fa fa-search" aria-hidden="true"></i><div id="search-results-loading" style="display: none;"><i class="fa fa-circle-o-notch fa-spin fa-fw"></i></div></button>
