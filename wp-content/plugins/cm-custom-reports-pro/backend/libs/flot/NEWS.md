...
### Bug fixes ###
 ...
 - Fixed a regression in text displaying, introduced in 0.8, that caused axis
 ...
 - Fixed a regression in text displaying, introduced in 0.8, where axis labels
...
A new plugin, jquery.flot.canvas.js, allows axis tick labels to be displayed
...
browser using HTML, then re-display them to canvas for export as an image.

The plugin tries to remain as faithful as possible to the original HTML display,
and goes so far as to automatically extract styles from CSS, to avoid having to
provide a separate set of styles when displaying to canvas. Due to limitations
...
### Changes ###
 ...
 - Added a canvas plugin to enable displaying axis tick labels to the canvas.
...
### Bug fixes ###
 ...
 - Fixed sub-pixel displaying issues with crosshair and selection lines.
   (patches by alanayoub and Daniel Shapiro, pull requests #17 and #925)

 - Fixed displaying issues when using the threshold plugin with several series.
...
## Changes ##
 ...
 - Image plugin: plot predisplayed images.
