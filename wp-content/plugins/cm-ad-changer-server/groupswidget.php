 ...
 */
class CMACGWidget extends WP_Widget
{
    ...
    {
        $widget_opts = array(
            'title'             => 'CM Ad Changer Group',
            'description'       => 'Display CM Ad Changer Banner Group',
            'classname'         => 'CMACG_AdChangerWidget',
            'group_id' => '0',
            'linked_banner'     => '1',
            'css_class'         => '',
            'custom_css'        => '',
        ...
        parent::__construct('cmacg_adchangerwidget', 'CM Ad Changer Group', $widget_opts);
    }
    ...
    function widget($args, $instance)
    {
        ...
        {
            $shortcode = '[cm_ad_changer group_id=' . $instance['group_id'];
    ...
    function update($new_instance, $old_instance)
    {
        ...
        if( !is_null($new_instance['group_id']) )
        {
    ...
    function form($instance)
    {
        ...
        <table cellpadding=2>
            ...
            echo '<tr><td><label for="' . $this->get_field_id('group_id') . '">Group ID</label></td>';
            echo '<td><input type="text" id="' . $this->get_field_id('group_id') . '" name="' . $this->get_field_name('group_id') . '" value="' . (isset($instance['group_id']) ? $instance['group_id'] : '') . '" size=4  /></td></tr>';
            ...
            echo '<td><input type="checkbox" id="' . $this->get_field_id('no_responsive') . '" name="' . $this->get_field_name('no_responsive') . '" ' . (isset($instance['no_responsive']) && $instance['no_responsive'] == '1' ? 'checked=checked' : '') . ' value="1"  /></td></tr>';
            ?>
        </table>
        <?php
    }
...
// registering widget
function cmacg_register_widget()
{
    register_widget('CMACGWidget');
}

add_action('widgets_init', 'cmacg_register_widget');
